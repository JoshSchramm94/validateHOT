#' Function to calculate precision of confusion matrix for general product
#' demand
#'
#' @description
#' `precision()` is one of the 5 metrics of the confusion matrix
...
#'
#' @details
#' The current logic of `precision()` is to determine whether a binary-coded
...
#' underestimates the purchase likelihood). The `precision()` function can be
#' useful for assessing the precision of that 2x2 matrix.
#'
...
precision <- function(data, group, opts, choice, none) {
  ...
  precision_data <- data %>%
    dplyr::mutate(
      ...
      # mark whether it is a purchase option
