...
#' function.
#'
#' `group` optional grouping variable, if results should be display by
#' different conditions. Has to be column name of variables in `data`.
#'
...
precision <- function(data, group, opts, choice, none) {
  ...
  precision_data <- data %>%
    ...
    dplyr::reframe(
      precision = 100 * (sum(buy == 1 & pred == 1) /
        (sum(buy == 1 & pred == 1) +
          sum(buy == 2 & pred == 1)))
    )
