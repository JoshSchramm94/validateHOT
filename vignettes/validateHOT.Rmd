...
<code>validateHOT</code> is a package for conjoint and MaxDiff users and 
its functions are threefold: First, <code>validateHOT</code> provides functions 
to validate a validation/holdout task and ensure whether the model is valid in 
terms of predicting outside choice sets (i.e., choices that were not included 
in the model's estimation). Second, <code>validateHOT</code> provides functions 
to run market simulations. Third, <code>validateHOT</code> also converts raw 
...
-   ... how to create utility scores for a validation/holdout task of a *CBC*, 
...
validation/holdout task in research papers, these are:
...
-   ... how to use <code>validateHOT</code> for simulation purposes
    ...
    frequency; <code>turf()</code>)
    -   Frequency of how many products are bought on average of a specified 
    assortment (<code>freqassort()</code>)
    -   How many participants are reached with a specified assortment (buying 
    at least one alternative of the assortment; <code>reach()</code>)
    -   running market simulations with specified set (<code>marksim()</code>). 
    <code>validateHOT</code> currently provides market simulations based on 
    share of preference as well as first choice rule.
-   ... and finally, how to use <code>validateHOT</code> to convert raw 
utilities into scores that are easier to interpret
    -   <code>att_imp()</code> (attribute importance for CBC and ACBC)
    -   <code>prob_scores()</code> (probability scores for (unanchored) 
    MaxDiff)
    -   <code>zc_diffs()</code> (zero-centered diffs for CBC and ACBC)
    -   <code>zero_anchored()</code> (zero-anchored interval scores for 
    (anchored) MaxDiff)
...
Moreover, we also load two packages for later purposes. We also explain some 
functions using the `tidyverse` logic. To do so, we load the `dplyr` 
(Wickham et al., 2023) package as well as the `magrittr` 
(Bache & Wickham, 2022) package.
...
At first, we have to create the validation/holdout task, which means that we 
need to create the utilities for each alternative presented in the 
validation/holdout task. This is quite easy for a *MaxDiff*, however, for a 
*CBC* or *ACBC*, we need to add the utilities of each attribute level and might 
even have to interpolate some values, if they are *linear-coded* or 
*piecewise-coded*.

### MaxDiff

However, let us start with the `MaxDiff`. We use the data set `MaxDiff` that 
comes with the package and store the utilities in the data set `HOT`. In this 
example, we assume that we have a validation task with seven alternatives plus 
a no-buy alternative (we define the argument by specifying the column index; 
`none = 19`), so a total of eight alternatives a participant can choose from. 
As already mentioned, it is a *MaxDiff* study (`method = "MaxDiff"`). 
Furthermore, we specify the column index of both the `id` (unique identifier 
in the data set) and `choice` (the actual choice in the validation/holdout 
task). To specify the seven alternatives in the validation/holdout task, we 
...
In case you have a forced-choice validation/holdout task, do not specify the 
`none` argument. Finally, in case you want to create a market scenario and not 
a validation/holdout task, you do not need to define the `choice` argument.
...
`validateHOT`. This example shows a *CBC* with three alternatives plus the 
no-buy alternative.

When using *CBC* or *ACBC*, we need to specify the coding, where `0` stands for 
part-worth coding, `1` for linear coding, and `2` for piecewise coding. Since 
we only have part-worth coding we set `coding = c(0, 0, 0)`.

Finally, we specify the attribute levels for each alternative. For example, 
the first alternative is composed of the following attribute levels 
`r colnames(CBC)[4]`, `r colnames(CBC)[9]`, and `r colnames(CBC)[19]`. 
The second is composed of the attribute levels `c(8, 12, 17)`, namely, 
`r colnames(CBC)[8]`, `r colnames(CBC)[12]`, and `r colnames(CBC)[17]`; etc.
...
createHOT(
  ...
  none = "none",
  coding = c(0, 0, 0),
  prod.levels = list(c("Att1_Lev1", "Att2_Lev1", "Att3_Lev6"), 
                     c("Att1_Lev5", "Att2_Lev4", "Att3_Lev4"), 
                     c("Att1_Lev2", "Att2_Lev2", "Att3_Lev4")),
  method = "CBC",
  choice = "HOT"
)
```
...
example above, however, this time we change the coding to 
`coding = c(0, 0, 1)`, which tells `createHOT()` that the first two attributes 
are part-worth coded, while the last attribute is linear-coded. How will this 
impact the way we define our products? We can see that when we specify the 
third attribute for our three alternatives, we insert a value that this 
alternative should have for the linear-coded attribute In this example, let us 
assume that the third attribute is weight. The first alternative in our 
validation/holdout task is assumed to have a weight of 60 (`c(4, 9, 60)`), the 
second a weight of `40`, and the last alternative a weight of `45`.

`createHOT()` will interpolate these values, therefore, we have to define the 
values we used for the attributes in the hierarchical Bayes estimations.
...
example of `CBC_lin`, there are seven levels ranging from 10 to 70. Please make 
sure to include **all** levels of the linear-coded attribute. Next, we have to 
specify the column index of the linear-coded attribute in the `lin.p` argument.

Finally, in our example, we would also like to keep some variables that should 
be attached to the data frame we are creating. We can specify the variables we 
would like to keep in `varskeep`.

> [**Important**]{.underline}: The value you would like to interpolate (in the 
example below 60, 40, and 45) need to be within the range you specify in 
`ìnterpolate.levels`. For example, instead of using `c(10, 20, 30, 40, 50, 
60, 70)`, we could also use `c(1, 2, 3, 4, 5, 6, 7)` or even better if we 
zero-center it to `c(-3, -2, -1, 0, 1, 2, 3)`. In this case we would need to 
adapt the values to be interpolate as well, so in our case 60 becomes 2, 40 
becomes 0, and 45 becomes 0.5 in a zero-centered approach. Usually these steps 
are done automatically by the software and you provide the attribute levels 
that were also shwon to the participants.

```{r}
HOT <- createHOT(
  # data frame
  data = CBC_lin,
  # column index of unique identifier
  id = 1, 
  # column index of none alternative
  none = 15, 
  # coding of the 3 attributes; 0 = part-worth, 1 = linear, 2 = piecewise
  coding = c(0, 0, 1), 
  # column index of the attribute levels for each alternative 
  # (for linear coded we specify the value to be interpolated)
  prod.levels = list(c(4, 9, 60), c(8, 12, 40), c(5, 10, 45)), 
  # actual values for the levels that should be interpolated
  interpolate.levels = list(c(10, 20, 30, 40, 50, 60, 70)), 
  # column index of the linear coded variable
  lin.p = 14, 
  # method
  method = "CBC", 
  # column index of variables that should be kept in the data frame
  varskeep = 17, 
  # column index of choice
  choice = 16
)
```

> [**Important**]{.underline}: Remember, since we have a linear-coded variable 
included, we **have** to provide the column indexes in `prod.levels`. For the 
other arguemnts (in the case above `id`, `none`, `lin.p`, `varskeep`, and 
`choice`) we could still specify the column names.

### Adaptive Choice-Based Conjoint (ACBC)

Finally, let us introduce you how to use `validateHOT` for an adaptive 
...
the `coding` argument for the `createHOT()` function. Since it is a little bit 
...
`validateHOT`. If you take a look at the data frame, you see that there are a 
total of eight attributes, plus the price (2 coefficients; `Price_2.093` as 
well as `Price_27.287`), and lastly the `none` coefficient. Since all eight 
attributes are part-worth coded, the setup is the same as for the CBC.

For our example, we use a validation/holdout task with six alternatives plus a 
no-buy alternative. First, we define the six alternatives. We can see that for 
`prod1`, we again indicate the column indexes for the eight different 
attributes. Finally, the price (last element of the vector, e.g., `prod1` has 
a price of `15.99`) is piecewise-coded.

After defining each alternative in the validation/holdout task, let us look at 
the `createHOT()` function. We specify our `data`, the column index of `id`, 
the column index of the no-buy alternative (`none`), the products, which we 
have defined before, the method (`ACBC`), and the column index of the actual 
choice (`choice`). We specify the coding (`c(0, 0, 0, 0, 0, 0, 0, 0, 2)`). 
Remember that in `createHOT()` we specify a linear-coded price in an ACBC as 
piecewise, which is why we set the last attribute to `2` instead of `1`. For 
`interpolate.levels`, we specify the lower bound of our price (`2.093`) and 
the upper bound of price (`27.287`). Finally, we specify `piece.p`, which is 
...
we specify the position of them. Please be aware that you have to specify the 
position for each alternative in the validation/holdout task, which is why you 
have to specify the position `c(35, 36)` for each of the six alternatives.

```{r, eval = F}
# define alternatives, for each attribute we specify the column index except 
# for the last one we specify the price to be interpolated.
...
HOT <- createHOT(
  ...
  # coding of the 9 attributes; 0 = part-worth, 1 = linear, 2 = piecewise
  coding = c(0, 0, 0, 0, 0, 0, 0, 0, 2), 
  # actual values for the levels that should be interpolated, for linear 
  # coded price in ACBC this is just the lower bound and the upper bound
  interpolate.levels = list(c(2.093, 27.287)), 
  # position of the piecewise coded variable
  piece.p = list(c(35, 36), c(35, 36), 
                 c(35, 36), c(35, 36), 
                 c(35, 36), c(35, 36)), 
   # method
  method = "ACBC",
...
So far, we have only looked at examples where at maximum one attribute was not 
...
to the previous one (six alternatives plus a no-buy alternative). We will use 
the data frame `ACBC_interpolate`, which is also part of `validateHOT`.
...
Again our validation/holdout task has six alternatives plus the no-buy 
...
(`c(3, 5, 8, 10)`), and the second vector contains the break points you have 
specified for your piecewise-coded price. In `ACBC_interpolate`, we provide an 
example where we used seven break points for the price 
(`c(1.99, 6.99, 9.99, 10.99, 12.99, 17.99, 25.99)`). We specify the column 
...
for the seven break points (`c(1.99, 6.99, 9.99, 10.99, 12.99, 17.99, 25.99)`) 
are stored in the columns 32 to 38. Again, we need to specify the lower and 
upper break point of the value we would like to interpolate in `piece.p`, and 
we need to do this for each of the six alternatives in the validation/holdout 
task. For example, for the first alternative (`15.99`), we see that the lower 
break point is `12.99` while the upper break point is `17.99`. Therefore, we 
specify the column index of these two in `piece.p` (`c(36, 37)`). Let us also 
look at the second product since it represents a scenario you might face in 
practice. The alternative should have a price of `12.99`. If we look at the 
break points we can see that one of the break points is also `12.99`. In this 
case, we could either use the column index of `12.99` as lower or upper 
position, the results will be identical.
...
HOT <- createHOT(
  ...
  prod.levels = list(
    # column index of the attribute levels for each alternative 
    # (for linear coded and piecewise coded we specify value to be interpolated)
  ...
  # actual values for the levels that should be interpolated 
  # (first entry refers to the linear coded variable, second entry refers 
  # to the piecewise coded variable)
  ...
  # position of the linear coded variable
  lin.p = 9, 
  # method
  method = "ACBC", 
  # column index of choice
...
like to create a validation/holdout task with an alternative-specific attribute 
please apply `NA` for this attribute level. That is all you have to change.

## Validation metrics

First, we focus on metrics that are often reported for validation/holdout tasks 
...
Once the validation/holdout task is created, the setup is similar for all three 
...
groups, we will use the `varskeep` argument.
...
For `hitrate()`, we specify our data (`HOT` in our case), the column names of 
alternatives (this includes the no-buy alternative; `opts`), and finally, the 
actual choice in the holdout task (`choice`).
...
Next, we will take a look at the five metrics of the confusion matrix. The 
...
validation/holdout task with five alternatives plus the alternative not to buy 
any of those chosen. `validateHOT` now measures whether or not a buy 
(participant opts for one of the five alternatives) or a no-buy (participant 
...
only difference is that you have to specify the column index of the no-buy 
alternative (`none`).

### `accuracy()`

We begin with the `accuracy()` and we still work with the `HOT` data frame we 
created above. We specify the eight alternatives (seven alternatives + no-buy 
...
to display the results split by the `Group` variable in `HOT`, therefore, we 
...
Next, let us talk about the simulation metrics. `validateHOT` currently 
...
approach. In this case, each alternative which utility exceeds the one of the 
threshold is considered as, for example, purchase option. In our case, we use 
the utility of the outside good (i.e., no-buy), however, it could also be the 
utility of a status quo alternative. A participant is considered as reached, if 
at least one alternative exceeds this threshold.
...
find out which combination of a total of three alternatives can reach most of 
the participants, put differently, the combination of three alternatives from 
which most of the participants buy at least one alternative. We use the 
`MaxDiff` data set for this example.

In this example, we have 16 different alternatives included as well as outside 
...
product assortment should include a total of three items (`size = 3`). Finally, 
...
By default, the results are sorted in descending order by `reach`. We see that 
`Combo 1` has a `reach` of 
`r round(turf(data = MaxDiff, opts = c(Option_01:Option_16), none = none, size = 3, approach = "thres") %>% select(reach) %>% .[1,1], digits = 2)`%. This assortment includes 
`Option_01`, `Option_04`, and `Option_13`. This means that for 
`r round(turf(data = MaxDiff, opts = c(Option_01:Option_16), none = none, size = 3, approach = "thres") %>% select(reach) %>% .[1,1], digits = 2)`% of the participants at least one of 
three items exceed the threshold. Finally, the frequency `freq` of this 
assortment is `r round(turf(data = MaxDiff, opts = c(Option_01:Option_16), none = none, size = 3, approach = "thres") %>% select(freq) %>% .[1,1], digits = 2)`.

If we decide to rather use the first choice approach, we simply change the 
`approach` argument to `approach = 'fc'`.
...
We see that the combination with the highest reach has changed. The combination 
with the highest reach (`reach` equal to `r round(turf(data = MaxDiff, opts = c(Option_01:Option_16), none = none, size = 3, approach = "fc") %>% select(reach) %>% .[1,1], digits = 2)`%) 
includes `Option_01`, `Option_04`, and `Option_06`. Please be aware that, if 
`approach = 'fc'`, `reach` and `freq` will be equal since the volume is limited 
...
We see that adding `Option_01` and `Option_4` yields the highest reach. Again, 
...
Argument `prohib` has to be list. Moreover, it could also include single 
...
In order to use `turf()`, we need to provide a threshold to the data frame, a 
...
this case, the threshold **has** to be lower, for example, 3.9999). In our 
...
Finally, `validateHOT` also provides a function, `marksim()` to calculate the 
assumed market share of a specified assortment. <code>validateHOT</code> 
...
easily decide by specifying the <code>method</code> argument.
...
Let us again use the examples from above, namely, `CBC`, `CBC_lin`, `ACBC`, and 
`ACBC_interpolate`.
...
Next, we assume that one of the attributes was linear-coded (`Att3_Lin`), 
which we specify in `coding`. Again, as we had to do for `createHOT()` as well, 
...
Above, we explained how to address a piecewise-coded ACBC in `createHOT()`. We 
...
Finally, in `ACBC_interpolate` we also had one linear-coded atribute, which we 
