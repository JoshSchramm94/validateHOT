...
works with other packages, for example, the ChoiceModelR package
[@ChoiceModelR]. Further, the validateHOT package is useful for
...
attributes or simulate markets [@gilbride2008; @steiner2018]. Their
ultimate goal is to predict future behavior [@green1990]. To get valid
results and results one can rely on, it is essential that the collected
data is valid and can also predict outside tasks that were not included
in the estimation of the utility scores. Including validation tasks is
highly recommended [@Orme2015; @rao2014]. They do not only check whether
your data is valid but can also help to test different models. The
validatHOT package provides helpful tools, to run these functions (i.e.,
validate a validation task, run market simulations, and communicate
results of a preference measurement technique) in an open-source tool.
...
according to the additive utility model [@rao2014, p. 82].
...
-   `kl()`: Kullback-Leibler-Divergence measures the divergence between
    ...
    [@ding2011; @philentropy]. The output provides both divergence
    between predicted from observed and observed from predicted due to
    the asymmetry of the Kullback-Leibler divergence. The validateHOT
...
-   `freqassort()`: Similar to `turf()`, `freqassort()` will give you
    the averaged frequency, how many products the participants will
    choose from a potential assortment. Again, you have to define a
    `none` alternative, because `freqassort()` uses the *threshold*
    approach, meaning if the utility of one product is above the utility
    of `none`, it is marked as potential purchase option
...
-   `reach()`: Similar to `turf()`, `reach()` will give you the averaged
    ...
    the products resemble a purchase option) with your in the function
...
-   `marksim()`: Runs market simulations (either share of preference,
    ...
    the lower and upper confidence interval, which is calculated
...
-   `acbc`: Example data set with raw utilities of an ACBC study
    conducted in Sawtooth. Price was linear-coded while the other
...
-   `cbc_linear`: Example data set with raw utilities of a CBC study
    conducted in Sawtooth. One attribute was linear-coded while the
    other attributes are part-worth coded [@sablotny-wackershauser2024;
    @sawtooth2024].

-   `maxdiff`: Example data set with raw utilities of a MaxDiff study
    conducted in Sawtooth [@sawtooth2024; @schramm2024].
...
`Metrics` by @Metrics), however, these packages need some more data
...
validation task. Again, the missing experience in *R* could be one
...
because they do not know how to use it correctly. Having a package to
