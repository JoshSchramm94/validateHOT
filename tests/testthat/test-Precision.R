...
test_that("Error if choice is missing ", {
  expect_error(precision(data = hot, opts = c(option_1:none), none = none))
})
...
test_that("group output equals group input - labelled input ", {
  ...
  expect_true(labelled::is.labelled(precision(
    data = hot,
...
test_that("precision() also working with data.frame not
...
test_that("Test whether results equals Metrics::precision ", {
  ...
  expect_equal(
    round(as.numeric(precision(
      data = hot,
      opts = c(option_1:none),
      choice = choice,
      none = none
    )), digits = 2),
    round(Metrics::precision(actual, predicted) * 100, digits = 2)
  )
})
# end --------------------------------------------------------------------------
