...
test_that("Error if choice is missing ", {
  expect_error(rmse(data = hot, opts = c(option_1:none)))
})
...
test_that("group output equals group input - labelled input ", {
  ...
  expect_true(labelled::is.labelled(rmse(
    data = hot,
    opts = c(option_1:none),
    choice = choice,
    group = group2
...
test_that("rmse() also working with data.frame not created
          with create_hot() ", {
  set.seed(2023)
