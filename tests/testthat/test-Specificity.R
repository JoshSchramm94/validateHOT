...
test_that("Error if choice is missing ", {
  expect_error(specificity(data = hot, opts = c(option_1:none), none = none))
})
...
test_that("group output equals group input - labelled input ", {
  ...
  expect_true(labelled::is.labelled(specificity(
    data = hot,
...
test_that("specificity() also working with data.frame not
