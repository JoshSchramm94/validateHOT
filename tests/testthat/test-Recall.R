...
test_that("Error if choice is missing ", {
  expect_error(recall(data = hot, opts = c(option_1:none), none = none))
})
...
test_that("group output equals group input - labelled input ", {
  ...
  expect_true(labelled::is.labelled(recall(
    data = hot,
...
test_that("recall() also working with data.frame not
...
test_that("Test whether results equals Metrics::recall ", {
  ...
  expect_equal(
    round(as.numeric(recall(
      data = hot,
      opts = c(option_1:none),
      choice = choice,
      none = none
    )), digits = 2),
    round(Metrics::recall(actual, predicted) * 100, digits = 2)
  )
})
# end --------------------------------------------------------------------------
