...
test_that("Error if epsilon is not numeric ", {
  expect_error(kl(
    data = hot,
    opts = c(option_1:none),
    choice = choice,
...
test_that("Error if log_base is not log or log2 is not numeric ", {
  expect_error(kl(
    data = hot,
    opts = c(option_1:none),
    choice = choice,
...
test_that("Error if length of opts equals 1 ", {
  hot_testthat <- hot
...
test_that("kl() also working with data.frame not created with create_hot() ", {
  set.seed(2023)

  new_hot_df <- data.frame(
    option_1 = runif(100, min = -5, max = 5),
    option_2 = runif(100, min = -5, max = 5),
    option_3 = runif(100, min = -5, max = 5),
    option_4 = runif(100, min = -5, max = 5),
    option_5 = runif(100, min = -5, max = 5),
    choice = sample(c(1:5), 100, replace = T)
  )
...
test_that("If log_base not specified, default is log ", {
  test1 <- kl(
    data = hot,
    opts = c(option_1:none),
    choice = choice,
