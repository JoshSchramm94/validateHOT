...
test_that("Error if choice is missing ", {
  expect_error(medae(data = hot, opts = c(option_1:none)))
})
...
test_that("group output equals group input - labelled input ", {
  ...
  expect_true(labelled::is.labelled(medae(
    data = hot,
    opts = c(option_1:none),
    choice = choice,
    group = group2
...
test_that("medae() also working with data.frame not created
          with create_hot() ", {
  set.seed(2023)
