...
test_that("Structure of Output tibble ", {
  expect_true(tibble::is_tibble(mae(
    data = HOT, opts = c(Option_1:None),
    choice = choice
  )))
})
...
test_that("group output equals group input - labelled input ", {
  ...
  expect_true(labelled::is.labelled(mae(
    data = HOT, opts = c(Option_1:None),
    choice = choice, group = Group2
  )[[1]]))
})
...
test_that("check whether examples are correct ", {
  expect_equal(round(as.numeric(mae(
    data = HOT,
    opts = c(Option_1:None),
    choice = choice
  )), 2), 4.93)
  expect_equal(
    round(as.numeric(mae(
      data = HOT,
      opts = c(Option_1:None),
      choice = choice,
      group = Group
